...
Texture::Texture(const std::string& texPath, const std::string& samplerName, ShaderProgram* shader_ptr) : 
    m_RendererID(0), m_Width(0), m_Height(0), m_Channels(0), m_ShaderPtr(shader_ptr)
{
...
unsigned char* Texture::LoadTexture(const std::string& path) {
    ...
    if( !data ) std::cout << "Could not load texture: " << path << std::endl;
    else {
